...
class PurgoMalumServiceWireMockTest {
    @Autowired
    ...
    @MethodSource("isCleanTextParameters")
    void isCleanTextTest(
            @NonNull final String given,
            final boolean expected
    ) {
        final boolean actual = purgoMalumService.isCleanText(given);
    ...
    @DisplayName("비속어 포함 여부 테스트")
    @MethodSource("isCleanTextParameters")
    void isNotCleanTextTest(
            @NonNull final String given,
            final boolean expected
    ) {
        final boolean actual = purgoMalumService.isNotCleanText(given);
        assertThat(actual).isNotEqualTo(expected);
    }

    private static Stream<Arguments> isCleanTextParameters() {
        return Stream.of(
                Arguments.of("badword", false),
                Arguments.of("goodword", true)
        );
    }
}